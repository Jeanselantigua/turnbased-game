...
**Current phase:** Phase 4 — Items / equipment (inventory, gear rolls, console camp)
...
3. **Dungeon mode** — `DungeonRun.run` fights waves with the same player `Character`s. `Dungeon.standard()` is 100 floors: normals (elites in the last 4 of each block), a boss every 15, opening enemy stats at 0.85, then +0.20 every 15 floors (not every wave). Solo / duo / trio multiply that scale by 0.75 / 0.90 / 1.00.
...
Inventory, ZZZ-style gear rolls, dungeon drops, and a console character menu. Set bonuses later.
...
- **Drops:** gold and chance-by-rank gear after each cleared wave (Normal 20%, Elite 75%, Boss guaranteed). BalanceSim ignores the bag.
- **Enhance cost:** cap +30. Total gold to max is still 3000 (the old +15 curve), spread so later levels cost more.
- **Console camp** between dungeon waves: view bag, equip/unequip, upgrade, leftover move points.

## Phase 5 — Polish: NOT STARTED

Presentation/UI beyond console, README, resume packaging. Augments land here as a visible skill tree (not a console-only Diablo roll).
